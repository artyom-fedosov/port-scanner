...
Scanner::Scanner(ipaddr_t const &ip, std::vector<char const *> const &ports) : m_ip{ip} {
    ...
    for (auto const prt : ports) {
        if (std::optional port{parsePort(prt)}) [[likely]]
            ...
            throw std::invalid_argument{std::string{prt} + " is not a valid port!"};
    }

    m_ports = std::move(tmpPorts);
}

[[nodiscard]] std::vector<std::pair<port_t, bool>> Scanner::scan() const {
...
[[nodiscard]] std::optional<port_t> Scanner::parsePort(char const *str) const noexcept {
    int prt;
    char const *strEnd{str + std::strlen(str)};
    std::from_chars_result const res{std::from_chars(str, strEnd, prt)};

    if (res.ec not_eq std::errc() or prt < MIN_PORT or prt > MAX_PORT or res.ptr not_eq strEnd)
        [[unlikely]]
        return {};
...
[[nodiscard]] bool Scanner::isPortAccessible(port_t const port) const {
    ...
    if (getaddrinfo(m_ip.c_str(), portStr.c_str(), &hints, &res) not_eq 0 or not res)
        return false;
    ...
    timeval constexpr timeout{TIMEOUT_MS / 1000, (TIMEOUT_MS % 1000) * 1000l};
